...
const SEVERITY_COLORS: Record<string, string> = {
  ...
  low: "text-blue-400", info: "text-cyan-400", none: "text-white/50",
};
...
function authorTag(r: ReportEntry) {
  ...
  return <span className="text-white/40 text-xs ml-1.5">({label})</span>;
}
        ...
        {report ? (
          <div className="mt-1.5 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {report.cvss != null && <span className="text-white/50 text-[13px]">CVSS {report.cvss}</span>}
              {report.id && <span className="text-white/35 font-mono text-[13px]">{report.id}</span>}
              {report.cve && <span className="text-white/50 font-mono text-[13px]">{report.cve}</span>}
              {report.cwe && <span className="text-white/50 font-mono text-[13px]">{report.cwe}</span>}
              {(report.agent_name || report.by_you) && (
                <span className="text-white/40 text-[13px]">{report.by_you ? "you" : report.agent_name}</span>
              )}
            </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-white/35 text-[13px]">({total})</span>
        {countEntries.map(([sev, n]) => (
          <span key={sev} className="text-[13px]">
            {sevBadge(sev)}<span className="text-white/50 ml-0.5">{n}</span>
          </span>
        ))}
      </div>
      {reports.length > 0 ? (
        <div className="mt-1.5 space-y-1">
          {reports.map((r, i) => (
            <div key={r.id ?? i} className="text-[13px]">
              ...
              {sevBadge(r.severity)}
              {r.id && <span className="text-white/35 font-mono ml-1.5">{r.id}</span>}
              <span className="text-white/60 ml-1.5">{r.title ?? "(untitled)"}</span>
              {authorTag(r)}
              {(r.target || r.endpoint) && (
                <div className="ml-3 text-white/40 font-mono text-xs">
      ...
      ) : <div className="mt-1 text-white/35 text-xs">No reports filed yet</div>}
    </div>
  );
}
