  ...
  return (
    <div>
      <div className="flex items-center gap-2">
        ...
        {filter && <span className="text-white/50 text-[13px]">{trunc(filter, 150)}</span>}
      </div>
      {requests.length > 0 && (
          ...
          {requests.slice(0, 20).map((r, i) => {
            ...
            return (
              <div key={i} className="flex gap-2">
                <span className={`w-10 shrink-0 font-bold ${METHOD_COLORS[m] ?? "text-white/50"}`}>{m}</span>
                <span className="text-white/45 truncate">{trunc(host + path, 180)}</span>
                {sc != null && <span className={`ml-auto shrink-0 ${statusColor(sc)}`}>{sc}</span>}
              </div>
            );
          })}
          {requests.length > 20 && <div className="text-white/35">... +{requests.length - 20} more</div>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center gap-2">
        ...
        {requestId != null && <span className="text-white/50 text-[13px]">#{requestId}</span>}
        {searchPattern && <span className="text-white/40 font-mono text-[13px]">/{trunc(searchPattern, 100)}/</span>}
      </div>
      {matches.length > 0 && (
        <div className="mt-1.5 font-mono text-[13px] space-y-1">
          {matches.slice(0, 5).map((m, i) => {
            ...
            return (
              <div key={i}>
                {before && <span className="text-white/35">...{before}</span>}
                <span className="text-amber-400/80 font-bold">{m.match}</span>
                {after && <span className="text-white/35">{after}...</span>}
              </div>
            );
          })}
          {matches.length > 5 && <div className="text-white/35">... +{matches.length - 5} more matches</div>}
        </div>
      )}
      {content && !matches.length && (() => {
        ...
        return (
          <CodeBlock className="text-white/40">
...
function SendRequest({ args, result }: ToolRendererProps) {
  ...
  return (
    <div>
      ...
      <div className="mt-1.5 font-mono text-[13px] space-y-0.5">
        <div>
          <span className="text-white/35 select-none mr-1">&gt;&gt;</span>
          <span className={`font-bold ${METHOD_COLORS[method] ?? "text-white/50"}`}>{method}</span>
          <span className="text-white/50 ml-1 break-all">{trunc(url, 180)}</span>
        </div>
        {headers && typeof headers === "object" && Object.entries(headers).slice(0, 5).map(([k, v]) => (
          <div key={k} className="text-white/35 pl-5">{k}: {sanitize(String(v), 150)}</div>
        ))}
      </div>
      {reqBody && (
        <CodeBlock className="text-white/50">{limitBody(reqBody, 4)}</CodeBlock>
      )}
      ...
      {statusCode != null && (
        <div className="font-mono text-[13px] mt-1.5">
          ...
          {responseTime != null && <span className="text-white/35 ml-2">{responseTime}ms</span>}
        </div>
      )}
      {resBody && (
        <CodeBlock className="text-white/40">{limitBody(resBody, 6)}</CodeBlock>
      )}
    </div>
  );
}
...
function RepeatRequest({ args, result }: ToolRendererProps) {
  ...
  return (
    <div>
      ...
      {modifications && typeof modifications === "object" && Object.keys(modifications).length > 0 && (
        <div className="mt-1.5 font-mono text-[13px] space-y-0.5">
          {Object.entries(modifications).slice(0, 5).map(([k, v]) => (
            <div key={k}><span className="text-orange-400/60">{k}:</span> <span className="text-white/45">{sanitize(typeof v === "string" ? v : JSON.stringify(v), 150)}</span></div>
          ))}
        </div>
      )}
      {statusCode != null && (
      ...
      {resBody && (
        <CodeBlock className="text-white/40">{limitBody(resBody, 5)}</CodeBlock>
      )}
    </div>
  );
}
...
function ScopeRules({ args }: ToolRendererProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {scopeName && <span className="text-white/50 text-[13px]">{trunc(scopeName, 50)}</span>}
    </div>
  );
}
...
function ListSitemap({ args }: ToolRendererProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {parentId && <span className="text-white/50 text-[13px]">under #{trunc(String(parentId), 20)}</span>}
    </div>
  );
}
...
function ViewSitemapEntry({ args }: ToolRendererProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {entryId && <span className="text-white/50 text-[13px]">#{trunc(String(entryId), 20)}</span>}
    </div>
  );
}
